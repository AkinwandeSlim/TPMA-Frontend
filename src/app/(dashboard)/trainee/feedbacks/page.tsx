...
const TraineeFeedbackPage = () => {
  ...
  }>({ supervisorName: "", lessonPlanTitle: "" });
  ...
  const {
    ...
    itemsPerPage: 10, // Matches backend ITEMS_PER_PAGE
  });
  ...
  const debouncedSetSearchQuery = useCallback(
    debounce((query: string) => {
      console.log("TraineeFeedbackPage - Debounced setSearchQuery:", {
        query,
        authLoading,
        currentSearchQuery: searchQuery,
      });
      if (authLoading) {
        console.log("TraineeFeedbackPage - Skipping setSearchQuery: auth still loading");
        return;
      }
      if (query === searchQuery) {
        console.log("TraineeFeedbackPage - Skipping setSearchQuery: query unchanged");
        return;
      }
  ...
  const debouncedFetchFeedback = useCallback(
    debounce(async (page: number, resetPage = false) => {
      console.log("TraineeFeedbackPage - Fetching feedback for page:", page, "resetPage:", resetPage);
      if (state.isFetching) {
        console.log("TraineeFeedbackPage - Skipping fetch: already fetching");
        return;
      }
      if (!user || !user.identifier) {
        console.log("TraineeFeedbackPage - Skipping fetch: user or user.identifier not available");
        setState((prev) => ({
          ...prev,
      ...
      try {
        ...
        console.log("TraineeFeedbackPage - Fetching feedback with params:", {
          userId: user.identifier,
          role: "teacherTrainee",
          page: pageToFetch,
          search: searchQuery || undefined,
          supervisorName: filterConfig.supervisorName,
        ...
        console.log("TraineeFeedbackPage - Backend response:", feedbackData);
        ...
        console.error("TraineeFeedbackPage - Error fetching feedback:", {
          message: err.message,
        ...
        if (err.response?.status === 400) {
          ...
          message = "No feedback found for your lesson plans.";
        }
        toast.error(message);
        setState((prev) => ({
          ...prev,
    ...
    [user, authLoading, router, searchQuery, filterConfig.supervisorName, filterConfig.lessonPlanTitle]
  );
  ...
  const handlePageChange = useCallback(
    (newPage: number) => {
      console.log("TraineeFeedbackPage - handlePageChange called:", newPage);
      if (
        newPage < 1 ||
  ...
  useEffect(() => {
    console.log("TraineeFeedbackPage - Auth useEffect running", {
      authLoading,
      user,
      authError,
      initialFetchDone,
    ...
    console.log("TraineeFeedbackPage - Token check:", { token: token ? "present" : "missing" });

    if (!token) {
      console.log("TraineeFeedbackPage - No token found, redirecting to signin");
      setState((prev) => ({
        ...prev,
    ...
    if (authLoading) {
      console.log("TraineeFeedbackPage - Auth loading, waiting...");
      setState((prev) => ({ ...prev, loading: true }));
      return;
    }

    if (authError) {
      console.log("TraineeFeedbackPage - Auth error detected:", authError);
      setState((prev) => ({ ...prev, error: authError, loading: false }));
      router.push("/auth/signin");
      return;
    }

    if (!user || user.role !== "teacherTrainee" || !user.identifier) {
      console.error("TraineeFeedbackPage - Invalid user or role:", { user });
      setState((prev) => ({
        ...prev,
        error: "Please sign in as a trainee to view feedback.",
        loading: false,
      }));
      toast.error("Access restricted to trainees.");
      router.push("/auth/signin");
      return;
    }

    if (!initialFetchDone) {
      console.log("TraineeFeedbackPage - Performing initial fetch");
  ...
  useEffect(() => {
    console.log("TraineeFeedbackPage - Search/Filter useEffect running", {
      searchQuery,
      supervisorName: filterConfig.supervisorName,
      lessonPlanTitle: filterConfig.lessonPlanTitle,
      initialFetchDone,
    ...
    if (
      ...
      user &&
      user.identifier &&
      (searchQuery || filterConfig.supervisorName !== "" || filterConfig.lessonPlanTitle !== "")
    ) {
      debouncedFetchFeedback(1, true);
    }
  }, [searchQuery, filterConfig.supervisorName, filterConfig.lessonPlanTitle, initialFetchDone, user, debouncedFetchFeedback]);

  const applyFilters = useCallback(() => {
    console.log("TraineeFeedbackPage - Applying filters:", tempFilterConfig);
    updateFilter("supervisorName", tempFilterConfig.supervisorName);
    updateFilter("lessonPlanTitle", tempFilterConfig.lessonPlanTitle);
    setShowFilterModal(false);
  }, [tempFilterConfig, updateFilter]);

  const clearFilters = useCallback(() => {
    console.log("TraineeFeedbackPage - Clearing filters");
    setTempFilterConfig({ supervisorName: "", lessonPlanTitle: "" });
    updateFilter("supervisorName", "");
    updateFilter("lessonPlanTitle", "");
    setShowFilterModal(false);
  }, [updateFilter]);

  const applySort = useCallback(() => {
    console.log("TraineeFeedbackPage - Applying sort:", tempSortConfig);
  ...
  const clearSort = useCallback(() => {
    console.log("TraineeFeedbackPage - Clearing sort");
  ...
  const feedbackColumns = useMemo(
    () => [
      ...
      { header: "Supervisor", accessor: "supervisorName", sortable: true, field: "supervisorName" },
      ...
      <div className="bg-red-100 text-red-700 p-4 rounded-md mb-4 max-w-7xl mx-auto">
        ...
        <div className="mt-4 flex gap-3">
          <button
            ...
            className="px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-md hover:from-blue-600 hover:to-indigo-700 transition-colors transform hover:scale-105"
            aria-label="Sign in"
          >
            Sign In
          </button>
          <button
            ...
            className="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition-colors transform hover:scale-105"
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-4 flex-col md:flex-row gap-4">
        <h1 className="text-lg font-semibold">Feedback on Your Lesson Plans</h1>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <TableSearch
            placeholder="Search by Lesson Plan, Supervisor, Comments..."
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={() => {
                console.log("TraineeFeedbackPage - Opening filter modal");
                setTempFilterConfig({
                  supervisorName: filterConfig.supervisorName || "",
                  lessonPlanTitle: filterConfig.lessonPlanTitle || "",
                });
            ...
            <button
              onClick={() => {
                console.log("TraineeFeedbackPage - Opening sort modal");
                setTempSortConfig({
      ...
      {state.feedback.length === 0 ? (
        <div className="bg-gray-50 p-6 text-center text-gray-500 rounded-md">
          <p className="mb-4">No feedback received for your lesson plans yet.</p>
        </div>
      ) : (
        <>
          <Table
          ...
          <div className="bg-white p-4 rounded-md w-[90%] md:w-[500px]">
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <input
                  value={tempFilterConfig.supervisorName}
                  onChange={(e) =>
                    setTempFilterConfig({ ...tempFilterConfig, supervisorName: e.target.value })
                  }
                  className="w-full p-2 border rounded-md"
                  aria-label="Filter by supervisor name"
                />
              </div>
              <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    console.log("TraineeFeedbackPage - Closing filter modal");
                    setShowFilterModal(false);
                  }}
          ...
          <div className="bg-white p-4 rounded-md w-[90%] md:w-[400px]">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    console.log("TraineeFeedbackPage - Closing sort modal");
                    setShowSortModal(false);
                  }}
                  className="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition-colors"
...
export default TraineeFeedbackPage;