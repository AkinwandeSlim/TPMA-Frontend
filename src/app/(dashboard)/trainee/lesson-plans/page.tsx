...
import { createLessonPlan, updateLessonPlan, deleteLessonPlan, getLessonPlans, generateLessonPlanPDF, generateAILessonPlan } from "@/lib/api";
...
const LessonPlanPage = ({ searchParams }: { searchParams: { [key: string]: string | undefined } }) => {
  ...
  const fetchLessonPlans = useCallback(
    async (page: number, resetPage = false) => {
      ...
      console.log("LessonPlanPage - Fetching lesson plans for page:", page, "resetPage:", resetPage);
      setIsFetching(true);
      setLoading(true);
      try {
        const pageToFetch = resetPage ? 1 : page;
        const response = await getLessonPlans(undefined, pageToFetch, 10, searchQuery, filterConfig.subject, filterConfig.status);
    ...
    [searchQuery, filterConfig.subject, filterConfig.status, currentPage, router]
  );
  ...
  useEffect(() => {
    ...
    console.log("LessonPlanPage - Token found in cookies:", token);

    if (!token) {
      const localStorageToken = localStorage.getItem("token");
      console.log("LessonPlanPage - Token found in localStorage:", localStorageToken);
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-4 flex-col md:flex-row gap-4">
        <h1 className="text-lg font-semibold">My Lesson Plans</h1>
      ...
      {memoizedLessonPlans.length === 0 && !error ? (
        <div className="bg-gray-50 p-6 text-center text-gray-500 rounded-md">
          <p className="mb-4">No lesson plans created yet. Create one now!</p>
          <FormModal
            table="lesson_plan"
            type="create"
            customSubmit={async (data: LessonPlanSchemaType) => {
