...
const FormModal = ({ table, type, data, id, refetch, customSubmit, ariaLabel }: Props) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      console.log(`FormModal - Triggering refetch for ${table} delete`);
      refetch?.();
      setOpen(false);
    } catch (error: any) {
  ...
  const handleRefetch = async () => {
    console.log(`FormModal - Triggering refetch for ${table} ${type}`);
    try {
      await refetch?.();
    } catch (error) {
      console.error(`FormModal - Error during refetch for ${table} ${type}:`, error);
    }
  };
